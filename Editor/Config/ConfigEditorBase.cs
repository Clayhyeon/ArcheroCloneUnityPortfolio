 ...
 public class ConfigEditorBase<T> : EditorWindow where T : EditorWindow
    {
        ...
        protected void DrawConfig<TT>() where TT : ConfigBase
        {
            ...
            if (_selectedProperty != null)
            {
                ...
                {
                    if (t.Id.ToString() != _selectedProperty)
                    {
                        continue;
                    }
                    ...
                    var area = GUILayoutUtility.GetRect(60, 60, GUIStyle.none, GUILayout.MaxWidth(60), GUILayout.MaxHeight(60));
                    GUI.Label(area, AssetPreview.GetAssetPreview(t.model));
        ...
        private void DrawSliderBar<TT>(IEnumerable<ConfigBase> prop) where TT : ConfigBase// 제너릭 타입으로 변경해야 함.
        {
            foreach (var configBase in prop)
            {
                var p = (TT) configBase;
